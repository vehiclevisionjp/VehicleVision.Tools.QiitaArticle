...
.EXAMPLE
    ...
    .\scripts\Build-CalendarExtension.ps1 -SkipBuild

    # 既存 vsix をインストールのみ
    .\scripts\Build-CalendarExtension.ps1 -InstallOnly
#>
param(
    [switch]$SkipBuild,
    [switch]$InstallOnly,
...
try {
    ...
    Get-ChildItem -Path $extensionDir -Filter 'VehicleVision.Tools.QiitaArticle.Calendar-*.vsix' | Remove-Item -Force
    # 固定名の既存ファイルも削除
    $vsixPath = Join-Path $extensionDir $vsixFile
