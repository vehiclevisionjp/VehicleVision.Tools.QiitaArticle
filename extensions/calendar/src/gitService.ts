...
export class GitService {
  ...
  getBranchFiles(): { success: boolean; files: string[] } {
    ...
    let diffResult = this.runGit(
      'diff --name-only --diff-filter=AM main -- public/',
    );
    if (!diffResult.success) {
      diffResult = this.runGit(
        'diff --name-only --diff-filter=AM master -- public/',
      );
    }
    if (diffResult.success) {
    ...
    const stagedResult = this.runGit(
      'diff --cached --name-only --diff-filter=AM -- public/',
    );
    ...
    const untrackedResult = this.runGit(
  ...
  mergeAndPush(): {
    ...
    if (!checkoutResult.success) {
      return {
        success: false,
        error: `main への切り替えに失敗しました: ${checkoutResult.output}`,
      };
    }
